...
def rr_preprocess(sig):
    predictions = detect_beats(sig, 200)
...
def stft_preprocess(sig, n_fft=40, new_fs=80, hop=20, stack_num=9):
    ...
    sig = butter_bandpass_filter(sig, 0.5, 40, 200, order=2)
    sig = signal.resample(sig, int((sig.shape[0] * new_fs)/200))
    sig = sig[20:-20]
...
def following_correlation(x):
    mod = x.shape[0] % 200
    if mod:
        c = np.corrcoef(x[:-mod].reshape(-1, 200))
    else:
        c = np.corrcoef(x.reshape(-1, 200))
    fc = c.diagonal(1)
    return np.var(fc)
...
def extract_basic_features(x, num, verbose=0):
    ...
    x = butter_bandpass_filter(x, 1, 25, 200, order=3)
    # predictions, values = det_elgendi(x, best_sig=0, weights=np.array([1]))
    predictions = detect_beats(x, 200)
    if predictions.size > 0:
    ...
    feat[ 0] = np.var(x)
    feat[ 1] = stats.skew(x)
    feat[ 2] = stats.kurtosis(x)

    if predictions.size > 3:

        feat[ 3:7] = perdiogram(x, 200)
        ...
        feat[ 7] = np.mean(qrs_areas)
        feat[ 8] = np.max(qrs_areas)

        feat[ 9] = np.mean(qrs_areas2)
        feat[ 10] = np.max(qrs_areas2)

        feat[ 11] = np.mean(qrs_malin)
        feat[ 12] = np.max(qrs_malin)

        feat[ 13] = np.mean(qrs_malin2)
        feat[ 14] = np.max(qrs_malin2)

        feat[ 15] = np.mean(n_plus)
        feat[ 16] = np.max(n_plus)

        feat[ 17] = np.mean(n_plus2)
        feat[ 18] = np.max(n_plus2)

        feat[ 19] = np.mean(v40)
        feat[ 20] = np.max(v40)

        feat[ 21] = np.mean(v40_2)
        feat[ 22] = np.max(v40_2)

        feat[ 23] = frequency_ratio(x, 1, 6, 200, 3)
        feat[ 24] = frequency_ratio(x, 1, 10, 200, 3)
        feat[ 25] = frequency_ratio(x, 8, 20, 200, 3)
        feat[ 26] = frequency_ratio(x, 1, 8, 200, 3)
        feat[ 27] = frequency_ratio(x, 12, 25, 200, 3)

        # RR features
        feat[ 28] = lorenz_plot_features(rr, rr_1)
        feat[ 29] = np.var(rr)
        feat[ 30] = np.var(rr_1)
        feat[ 31] = np.var(rr_2)
        ...
        feat[ 32] = -rr.dot(np.log(rr))
        feat[ 33] = -rr_1_abs.dot(np.log(rr_1_abs))
        se = sample_entropy(rr, 2, tolerance=None)
        feat[ 34] = se[0]
        feat[ 35] = se[1]
        if np.isnan(feat[ 34]) or np.isinf(feat[ 34]):
            feat[ 34] = 0
        if np.isnan(feat[ 35]) or np.isinf(feat[ 35]):
            feat[ 35] = 0
        feat[ 36] = shannon_entropy(rr)
        feat[ 37] = shannon_entropy(rr_1)

        # Other
        feat[38] = following_correlation(x)
        # b = perdiogram(rr, 200)
    return feat
...
#             feat[ind, 23] = frequency_ratio(x, 1, 6, 200, 3)
#             feat[ind, 24] = frequency_ratio(x, 1, 10, 200, 3)
#             feat[ind, 25] = frequency_ratio(x, 8, 20, 200, 3)
#             feat[ind, 26] = frequency_ratio(x, 1, 8, 200, 3)
#             feat[ind, 27] = frequency_ratio(x, 12, 25, 200, 3)
...
def find_noise_features(x):
    # from detector_elgendi import det_elgendi
    x = butter_bandpass_filter(x, 1, 35, 200, order=4, verbose=0)
    # predictions, values = det_elgendi(x, best_sig=0, weights=np.array([1]))
    predictions = detect_beats(x, 200)
    if predictions.size == 0:
        snr = 0
    ...
    fr = frequency_ratio(x, 1, 6, 200, 4)
    fr2 = frequency_ratio(x, 12, 20, 200, 4)
